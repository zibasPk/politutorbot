...
public static class WebServer
{
    public static void Init()
    {
        ...
        builder.Services.AddCors(p =>
            p.AddPolicy("corsapp",
                policyBuilder =>
                {
                    policyBuilder.WithOrigins(GlobalConfig.WebConfig!.AllowedCors).AllowAnyMethod().AllowAnyHeader();
                }));
        ...
        // Post endpoints
        app.MapPost("/api/tutoring/start/{tutorCode:int?}/{studentCode:int?}/{examCode:int?}", StartTutoringAction)
            .RequireAuthorization();
    ...
    private static async void StartTutoringAction(int? tutorCode, int? studentCode, int? examCode,
